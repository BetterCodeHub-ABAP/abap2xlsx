...
CLASS ZCL_EXCEL_COMMON IMPLEMENTATION.
  ...
  METHOD char2hex.
    ...
    IF zif_excel_common~o_conv IS NOT BOUND.
      zif_excel_common~o_conv = cl_abap_conv_out_ce=>create( endian   = 'L'
                                            ignore_cerr = abap_true
                                            replacement = '#' ).
    ENDIF.

    CALL METHOD zif_excel_common~o_conv->reset( ).
    CALL METHOD zif_excel_common~o_conv->write( data = i_char ).
    r_hex+1 = zif_excel_common~o_conv->get_buffer( ). " x'65' must be x'0065'

  ENDMETHOD.


  METHOD class_constructor.
    zif_excel_common~c_xlsx_file_filter = 'Excel Workbook (*.xlsx)|*.xlsx|'(005).
  ENDMETHOD.
...
* We skip it. for now.
  ...
  METHOD zif_excel_common~assert_differs.
...
* We do nothing for now not supported
  ...
  METHOD zif_excel_common~assert_equals.
    DATA: ls_seoclass TYPE seoclass.
...
* We do nothing for now not supported
  ...
  METHOD zif_excel_common~calculate_cell_distance.
...
*--------------------------------------------------------------------*
    zif_excel_common~convert_columnrow2column_a_row( EXPORTING
                                      i_columnrow = iv_reference_cell
                                    IMPORTING
                                      e_column    = lv_reference_col_alpha
                                      e_row       = lv_reference_row ).
    lv_reference_col = zif_excel_common~convert_column2int( lv_reference_col_alpha ).
...
*--------------------------------------------------------------------*
    ...
    lv_current_col = zif_excel_common~convert_column2int( lv_current_col_alpha ).
...
*--------------------------------------------------------------------*
  ...
  METHOD zif_excel_common~convert_column2alpha.
...
* Propagate zcx_excel if error occurs           " issue #155 - less restrictive typing for ip_column
    lv_column = zif_excel_common~convert_column2int( ip_column ).  " issue #155 - less restrictive typing for ip_column
...
*--------------------------------------------------------------------*
  ...
  METHOD zif_excel_common~convert_column2int.
...
*--------------------------------------------------------------------*
  ...
  METHOD zif_excel_common~convert_columnrow2column_a_row.
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
  ...
  METHOD zif_excel_common~convert_range2column_a_row.
*--------------------------------------------------------------------*
* issue #230   - Pimp my Code
...
* Initialize output parameters
    ...
    e_sheet = zif_excel_common~unescape_string( lv_sheet ).                  " Return in unescaped form
  ENDMETHOD.


  METHOD zif_excel_common~date_to_excel_string.
    ...
    IF ip_value > zif_excel_common~c_excel_1900_leap_year.
      lv_date_diff = ip_value - zif_excel_common~c_excel_baseline_date + 2.
    ELSE.
      lv_date_diff = ip_value - zif_excel_common~c_excel_baseline_date + 1.
    ENDIF.
    ep_value = zcl_excel_common=>zif_excel_common~number_to_excel_string( ip_value = lv_date_diff ).
  ENDMETHOD.


  METHOD zif_excel_common~describe_structure.
  ...
  METHOD zif_excel_common~determine_resulting_formula.
...
*--------------------------------------------------------------------*
    zif_excel_common~calculate_cell_distance( EXPORTING
...
*--------------------------------------------------------------------*
    ev_resulting_formula = zif_excel_common~shift_formula( iv_reference_formula = iv_reference_formula
  ...
  METHOD zif_excel_common~encrypt_password.
  ...
  METHOD zif_excel_common~escape_string.
*--------------------------------------------------------------------*
* issue #230   - Pimp my Code
...
*--------------------------------------------------------------------*
  ...
  METHOD zif_excel_common~excel_string_to_date.
    DATA: lv_date_int TYPE i.

    TRY.
        lv_date_int = ip_value.
        ep_value = lv_date_int + zif_excel_common~c_excel_baseline_date - 2.
        " Needed hack caused by the problem that:
        " Excel 2000 incorrectly assumes that the year 1900 is a leap year
        " http://support.microsoft.com/kb/214326/en-us
        IF ep_value < zif_excel_common~c_excel_1900_leap_year.
          ep_value = ep_value + 1.
        ENDIF.
  ...
  METHOD zif_excel_common~excel_string_to_number.
...
* But currently this works fine - even for numbers in scientific notation
  ...
  METHOD zif_excel_common~excel_string_to_time.
...
* We do nothing for now not supported
  ...
  METHOD zif_excel_common~get_fieldcatalog.
...
* just use the fieldname as description - that is better than nothing
  ...
  METHOD zif_excel_common~is_cell_in_range.
...
* Split range and convert columns
    zif_excel_common~convert_range2column_a_row(
      EXPORTING
        i_range        = ip_range
      IMPORTING
    ...
    lv_column_start_i = zif_excel_common~convert_column2int( ip_column = lv_column_start ).
    lv_column_end_i   = zif_excel_common~convert_column2int( ip_column = lv_column_end ).

    lv_column_i = zif_excel_common~convert_column2int( ip_column = ip_column ).
...
* Check if cell is in range
  ...
  METHOD zif_excel_common~number_to_excel_string.
  ...
  METHOD zif_excel_common~recursive_class_to_struct.
    " # issue 139
...
*        ENDIF.
          zcl_excel_common=>zif_excel_common~recursive_class_to_struct( EXPORTING i_source  = <attribute>
  ...
  METHOD zif_excel_common~recursive_struct_to_class.
    " # issue 139
* is working for me - but after looking through this coding I guess
* I'll rewrite this to a version w/o recursion
* This is private an no one using it so far except me, so no need to hurry
...
* Only borders will be passed as unbound references.  But since we want to set a value we have to create an instance
            ...
            <attribute> = o_border.
          ENDIF.
          zcl_excel_common=>zif_excel_common~recursive_struct_to_class( EXPORTING i_source  = <field>
                                                                 i_sourcex = <fieldx>
...
*        <attribute_s> = <field>.
  ...
  METHOD zif_excel_common~shift_formula.
...
*--------------------------------------------------------------------*
          TRY.
            ...
            CATCH cx_root.
              lv_errormessage = 'Internal error in Class ZCL_EXCEL_COMMON Method zif_excel_common~shift_formula Spot 1 '.  " Change to messageclass if possible
              RAISE EXCEPTION TYPE zcx_excel
                EXPORTING
                  error = lv_errormessage.
          ENDTRY.
...
*--------------------------------------------------------------------*
          TRY.
              lv_tcoln = zcl_excel_common=>zif_excel_common~convert_column2int( lv_tcol1 ) + iv_shift_cols.
            CATCH zcx_excel.
              CONCATENATE lv_cur_form lv_substr1 INTO lv_cur_form.
              lv_cnt = lv_cnt + 1.
              lv_offset1 = lv_cnt.
...
*--------------------------------------------------------------------*
            ...
            ELSE.
              TRY.
                  lv_tcol2 = zcl_excel_common=>zif_excel_common~convert_column2alpha( lv_tcoln ).
...
*--------------------------------------------------------------------*
  ...
  METHOD zif_excel_common~split_file.
...
** get only the filename
  ...
  METHOD zif_excel_common~time_to_excel_string.
    DATA: lv_seconds_in_day TYPE i,
          lv_day_fraction   TYPE f,
    ...
    ep_value = zcl_excel_common=>zif_excel_common~number_to_excel_string( ip_value = lv_day_fraction ).
  ENDMETHOD.


  METHOD zif_excel_common~unescape_string.
