...
METHOD get_filter_range.
  ...
  l_col_start_c = zcl_excel_common=>zif_excel_common~convert_column2alpha( ip_column = filter_area-col_start ) .
  l_col_end_c   = zcl_excel_common=>zif_excel_common~convert_column2alpha( ip_column = filter_area-col_end ) .
...
METHOD get_filter_reference.
  ...
  r_ref = zcl_excel_common=>zif_excel_common~escape_string( ip_value = l_value ).
